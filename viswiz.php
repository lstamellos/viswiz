...
function viswiz_render_graph_node_select( $name, $selected, $nodes = array(), $attributes = array() ) {
    ...
    <select name="<?php echo esc_attr( $name ); ?>" class="regular-text" <?php foreach ( $attributes as $attr => $value ) : ?><?php echo esc_attr( $attr ); ?>="<?php echo esc_attr( $value ); ?>" <?php endforeach; ?>>
        ...
        <?php foreach ( $nodes as $node_index => $node ) : ?>
            ...
            ?>
            <option value="<?php echo esc_attr( $node_id ); ?>" data-node-title="<?php echo esc_attr( $node_title ); ?>" <?php selected( $selected, $node_id ); ?>><?php echo esc_html( $option_label ); ?></option>
        <?php endforeach; ?>
...
function viswiz_render_visualization_meta_box( WP_Post $post ) {
    ...
    <div class="viswiz-tab-panel <?php echo $active_tab === 'formatting' ? 'is-active' : ''; ?>" data-viswiz-panel="formatting">
        ...
        <div class="viswiz-field-group" data-viswiz-types="graph,flow_diagram,org_chart">
            ...
            <p>
                <label><input type="checkbox" name="viswiz_meta[format_colors][show_node_images]" value="1" <?php checked( $meta['format_colors']['show_node_images'] ?? 1, 1 ); ?> /> Show node images on cards</label><br />
                <label><input type="checkbox" name="viswiz_meta[format_colors][show_type_badges]" value="1" <?php checked( $meta['format_colors']['show_type_badges'] ?? 1, 1 ); ?> /> Show type and subtype badges</label>
            </p>
        </div>
    </div>
